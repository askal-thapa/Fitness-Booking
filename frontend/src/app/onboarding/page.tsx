...
export default function OnboardingPage() {
  ...
  const finishOnboarding = async () => {
    ...
    try {
      ...
      // Update the session state to reflect onboarding completion
      await update({
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-charcoal flex flex-col items-center justify-center p-8 text-center space-y-6">
        <div className="w-16 h-16 border-4 border-muted-blue/20 border-t-muted-blue rounded-full animate-spin" />
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-off-white">Creating your personalized fitness plan...</h2>
          <p className="text-off-white/40">Analyzing your metrics and preferences</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-charcoal flex flex-col items-center py-12 px-4 md:py-20">
      <div className="max-w-xl w-full space-y-12">
        <div className="space-y-6">
          <div className="flex justify-between items-end">
            <div className="space-y-1">
              <span className="text-xs font-bold uppercase tracking-widest text-muted-blue">Step {step} of {STEPS.length}</span>
              <h1 className="text-3xl font-bold text-off-white">{STEPS[step - 1]}</h1>
            </div>
            <span className="text-sm font-medium text-off-white/40">{Math.round(progress)}% Complete</span>
          </div>
        ...
        <div className="min-h-[400px]">
          ...
          {step === 5 && (
            <div className="space-y-6">
              <p className="text-sm text-off-white/40">Select all that apply to you</p>
                  ...
                  <button
                    ...
                    className={`px-6 py-3 rounded-full border transition-all font-medium ${
                      formData.healthConditions.includes(condition)
                        ? "bg-muted-blue border-muted-blue text-white"
                        : "bg-charcoal-light border-white/10 text-off-white/60 hover:border-white/20"
                    }`}
                  >
                    {condition}
                  </button>
          ...
          {step === 6 && (
            <div className="space-y-8">
              <div className="space-y-4">
                <label className="text-sm font-medium text-off-white/60">Workout Type</label>
              ...
              <div className="space-y-4">
                <label className="text-sm font-medium text-off-white/60">Diet Preference</label>
                <div className="grid grid-cols-3 gap-3">
